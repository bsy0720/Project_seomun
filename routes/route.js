...
// 작성페이지
router.post('/store', 
[check('content').isLength({ min: 1, max: 6000})],
    ...
    router.get('/new', (req,res) => { // 이쪽의 /new는 공지사항수정페이지 form action의 이름
        ...
        db.getMemoById(id, (row) => {
            if (typeof id == 'undefined' || row.length <= 0) {
                ...
                res.render('new_notice_write',{row:row[0]});
            }
        })
    });
...
router.get('/new_notice_write', (req, res) => {
    ...
    // 공지사항 수정한걸 보내는거
    router.post('/new', [check('content').isLength({min: 1, max: 6000})],  (req, res) => { // 수정
        let errs = validationResult(req);
        let param = JSON.parse(JSON.stringify(req.body));
        let id = param['id'];
        let title = param['title'];
        ...
        if(errs['errors'].length > 0) {
            db.getMemoById(id, (row) => {
                res.render('new_notice_write', { row:row[0], errs:errs['errors']})
            }) 
        } else {
    ...
    let id = req.query.id;
        db.getMemoById(id, (row) => {
            if (typeof id == 'undefined' || row.length <= 0) {
                ...
                res.render('seomun_notice_page',{row:row[0]});
            }
        })
    });
